...
export const NavMenu = ()=> {
  ...
  const linkActive = (href: string)=> href === params
    ...
    return (
        ...
        <button onClick={()=> router.push(`?settings=true`)} className={`flex items-center w-full justify-center hover:bg-whited/10 p-3 ${linkActive('/setting') && 'border-l-4 border-whited dark:border-gray-700 bg-whited/10' }`}> <Cog8ToothIcon className="text-whited dark:text-gray-800" width={35} height={35} /> </button>
      </nav>
    )
}