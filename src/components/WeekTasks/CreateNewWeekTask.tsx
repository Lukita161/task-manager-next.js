...
export default function CreateNewWeekTask() {
  ...
  return (
    <>
      ...
      >
        <div className="fixed inset-0 flex w-screen items-center justify-center p-3 bg-blacked/50">
          <DialogPanel className="max-w-lg lg:w-[100vw] space-y-3 border rounded-xl bg-white p-12">
            ...
            <form action={handleAction} className="flex flex-col gap-2">
                    ...
                    <input className="w-full mb-4 shadow-sm p-4 border-b rounded bg-gray-100 border-gray-400 outline-none focus:border-b-2 focus:bg-whited/40 transition-colors" type="text" name="Name" placeholder="Nombre tarea" />

                    <label className="font-medium  text-gray-600" htmlFor="Description">Descripción de la tarea: </label>
                    <input className="w-full mb-4 shadow-sm p-4 border-b rounded bg-gray-100 bg-none border-gray-400 outline-none focus:border-b-2 focus:bg-whited/40 transition-colors" type="Description" name="Description" placeholder="Descripcion tarea" />

                    <label className="font-medium  text-gray-600" htmlFor="Categoria">Categoria: </label>
                    <select className="w-full mb-4 shadow-sm p-4 border-b rounded bg-gray-100 bg-none border-gray-400 outline-none focus:border-b-2 focus:bg-whited/40 transition-colors" name="Categoria">
                      {category.map(category => (
                        <option key={category.value} value={category.value}> {category.name} </option>
                      ))}
                    </select>
                    <label className="font-medium  text-gray-600" htmlFor="WeekDay">Asignale un día</label>
                    <select className="w-full shadow-sm p-4 border-b rounded bg-gray-100 bg-none border-gray-400 outline-none focus:border-b-2 focus:bg-whited/40 transition-colors" name="WeekDay">
                      {weekDays.map(day => (
